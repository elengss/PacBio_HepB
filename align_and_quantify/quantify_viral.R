...
#### fastqlines is a file that contains a vector of library sizes to be normalised by, generated from align.sh
#### Since each read spans across 4 lines, needs to be divided by 4 to get the number of reads
read.table("fastqlines",header=F)->fq
fq[,1]/4-> numreads
pdf("canonical_transcripts_D3L.start_only.norm_by_reads.pdf")
for(i in 1:length(all)){
df <- data.frame(transcript=d3n$V10,
                count=allcant[[i]]/numreads)
